...
resource_samples="$artifact_dir/${profile}-${stamp}.resources.tsv"
{
  ...
  echo "firebase=$(npx --yes firebase-tools --version 2>/dev/null | tail -1)"
  echo "rtdb_sync_commit=$(git rev-parse HEAD)"
  echo "workspace_rtdb_rs_commit=$(git -C ../rtdb-rs rev-parse HEAD 2>/dev/null || true)"
  echo "workspace_rtdb_typed_commit=$(git -C ../rtdb-typed rev-parse HEAD 2>/dev/null || true)"
  echo "workspace_rtdb_admin_commit=$(git -C ../rtdb-admin rev-parse HEAD 2>/dev/null || true)"
  echo "resolved_dependency_graph_begin"
  cargo tree --locked --all-features -e normal 2>/dev/null |
    rg 'rtdb-(rs|typed|admin) v' || true
  echo "resolved_dependency_graph_end"
  echo "host=$(uname -a)"
